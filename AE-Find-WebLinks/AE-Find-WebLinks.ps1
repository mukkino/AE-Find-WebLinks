Sorry Fabio — here is the rewritten Markdown block for the new version.

# Find-WebLinks

Find-WebLinks is a PowerShell command-line tool for extracting, filtering, saving, and optionally crawling web links from either a single web page or a text file containing many source URLs.
...
Find-WebLinks can optionally follow links discovered on source pages.
...
Find-WebLinks can enforce `robots.txt`, but this is disabled by default.
...
```powershell
.\Find-WebLinks.ps1 "https://example.com" "*download*" ".\matched-links.txt" New
```

### Scan many source URLs from a file

```powershell
.\Find-WebLinks.ps1 ".\source-urls.txt" "*download*" ".\matched-links.txt" New File
```

### Crawl one level on the same domain

```powershell
.\Find-WebLinks.ps1 "https://example.com" "*download*" ".\matched-links.txt" New -FollowDepth 1 -FollowScope SameDomain
```
...
```powershell
.\Find-WebLinks.ps1 "https://example.com" "*download*" ".\matched-links.txt" New -FollowDepth 2 -FollowScope SameDomain -EnforceRobotsTxt
```
...
Find-WebLinks is not a browser.
...
For JavaScript-rendered websites, use a browser-based crawler instead, or provide Find-WebLinks with URLs from already-rendered/exported pages.
